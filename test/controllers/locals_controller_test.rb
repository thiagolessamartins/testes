...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post :create, local: { cidade: @local.cidade, complemento: @local.complemento, endereco: @local.endereco, status: @local.status, uf: @local.uf }
    end
  ...
  test "should update local" do
    patch :update, id: @local, local: { cidade: @local.cidade, complemento: @local.complemento, endereco: @local.endereco, status: @local.status, uf: @local.uf }
    assert_redirected_to local_path(assigns(:local))
  end
